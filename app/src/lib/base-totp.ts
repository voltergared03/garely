...
export type TotpView = { set: boolean; code?: string; period?: number; remainingSec?: number };
...
export function totpCellView(stored: unknown, now = Date.now()): TotpView {
  ...
  return {
    set: true,
    code: totp(secret, now, TOTP_STEP),
    period: TOTP_STEP,
    remainingSec: TOTP_STEP - (Math.floor(now / 1000) % TOTP_STEP),
  };
}
