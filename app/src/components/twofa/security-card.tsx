...
export function TwoFactorSecurity({ enabled: initialEnabled }: { enabled: boolean }) {
  ...
  async function disable() {
    ...
    try {
      ...
      setError('Помилка мережі'); setBusy(false);
    }
  }
          ...
          <div style={{ fontSize: 13.5, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {enabled ? (
              <span className="chip" style={{ background: 'color-mix(in oklab, var(--green) 14%, transparent)', color: '#a7f3d0', borderColor: 'color-mix(in oklab, var(--green) 30%, transparent)' }}>
                <ShieldCheck size={11} /> Увімкнено
              </span>
            ) : (
              <span className="chip" style={{ color: 'var(--muted)' }}>Вимкнено</span>
            )}
          </div>
          <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>
            Код з додатку-автентифікатора при кожному вході
          </div>
        </div>
        {enabled ? (
      ...
      {mode && (
        ...
        >
          <div className="card" style={{ width: '100%', maxWidth: 420, padding: '22px 24px', position: 'relative' }}>
            <button className="btn btn-ghost btn-icon" onClick={close} style={{ position: 'absolute', top: 12, right: 12, width: 30, height: 30 }} aria-label="Закрити">
              <X size={16} />
            </button>
            ...
            {mode === 'setup' ? (
              ...
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 8 }}>Вимкнути 2FA?</div>
                <p style={{ color: 'var(--text-2)', fontSize: 13, lineHeight: 1.5, margin: '0 0 14px' }}>
                  Введіть поточний код з додатку (або резервний код), щоб підтвердити вимкнення.
                </p>
                <input
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                  <button className="btn" onClick={close} style={{ flex: 1, justifyContent: 'center' }}>Скасувати</button>
                  <button className="btn btn-primary" onClick={disable} disabled={busy || code.length < 6} style={{ flex: 1, justifyContent: 'center', background: 'var(--red)', borderColor: 'var(--red)' }}>
                    {busy ? <Loader2 size={15} className="spin" /> : 'Вимкнути'}
                  </button>
                </div>
              </div>
            )}
