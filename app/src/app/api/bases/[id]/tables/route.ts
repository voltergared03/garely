...
// POST /api/bases/[id]/tables — create a table (editor+). Bootstrapped with a
// primary text field ("Name") + a default grid view.
export const POST = withRoute('tables.create', async (req: NextRequest, ctx: Ctx) => {
  const r = await requireOrg();
  if (r instanceof Response) return r;
  const { id: baseId } = await ctx.params;
  const acc = await baseForOrg(baseId, r.orgId, r.session);
  if (!acc) return jsonError('not_found', 404);
  const g = await gate(acc, r.orgId, r.session, 'editor');
  if (g) return g;
