...
log_level: info
# room_composite (grid video) needs ~4 CPU by default; lower the cost so it is
# admitted within egress's modest CPU allotment (2 CPU, see docker-compose.yml)
# on small hosts — otherwise recording fails with "no response from servers".
cpu_cost:
